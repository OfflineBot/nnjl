...
function write_json_dense_layer(layer::DenseLayer, name::String, activ_name::String)
    weight_sizes = size(layer.weight)
    data = NNJson(weight_sizes[1], weight_sizes[2], activ_name, layer.weight, layer.bias)
    if !isdir("data")
        mkdir("data")
    end
